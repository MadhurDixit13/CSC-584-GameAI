    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                writeGameRecordsToFile("game_records.csv", gameRecords);
                window.close();
       ...
       if(!path.empty()){ 
        ...
        window.display();
        //float game record
        float distance = euclideanDistance(character.position, enemy.position);
        std::string enemy_action;
        if (enemySteering.linear.x == 0 && enemySteering.linear.y == 0 && euclideanDistance(character.position, enemy.position) > 250.f ) {
            enemy_action = "Dance";
        } else if (euclideanDistance(character.position, enemy.position) < 10.f) {
            enemy_action = "Colliding";
        } else {
            enemy_action = "Moving";
        }
        gameRecords.push_back({character.position.x, character.position.y, enemy.position.x, enemy.position.y, enemy_action, distance});
    }
