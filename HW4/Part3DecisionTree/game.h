...
//Recording game values
struct GameRecord {
    float character_x;
    float character_y;
    float enemy_x;
    float enemy_y;
    std::string enemy_action; // Could be "Wandering", "Following", etc.
    float distance; // Distance between character and enemy
};

void writeGameRecordsToFile(const std::string& filename, const std::vector<GameRecord>& records) {
    std::ofstream file(filename);
    if (file.is_open()) {
        // Write the header
        file << "character_x,character_y,enemy_x,enemy_y,enemy_action,distance\n";

        // Write the data
        for (const auto& record : records) {
            file << record.character_x << ","
                 << record.character_y << ","
                 << record.enemy_x << ","
                 << record.enemy_y << ","
                 << record.enemy_action << ","
